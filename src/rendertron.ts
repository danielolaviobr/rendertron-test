...
export class Rendertron {
  ...
  async initialize() {
    ...
    this.app.use(route.get("/preview/:id", this.handleGetPreview.bind(this)));
    this.app.use(route.get("/pdf/:id", this.handleGetPDF.bind(this)));
  ...
  async handleGetPreview(ctx: Koa.Context, id: string) {
    if (!this.renderer) {
    ...
    const url = `https://curriculo-show.vercel.app/pdf/preview/${id}`;
    try {
  ...
  async handleGetPDF(ctx: Koa.Context, id: string) {
    if (!this.renderer) {
      throw new Error("No renderer initalized yet.");
    }
    ...
    const mobileVersion = "mobile" in ctx.query ? true : false;
    const url = `https://curriculo-show.vercel.app/pdf/${id}`;
    try {
      console.log("Before puppeteer");
